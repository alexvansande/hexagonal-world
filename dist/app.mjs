...
import {decodeMapState,encodeMapState,distortionEnabled,restorePanelStates} from './map-state.mjs?v=url-state-1';
...
import {ReliefRenderer,reliefRanges,reliefDefaults,reliefLooks} from './relief.mjs';
...
void main(){if(felvClip==1){float fy=-flatPosition.y;float fx=flatPosition.x-sqrt(3.)*fy;if(fy<0.||fy>sqrt(3.)||fx< -1.||fx>5.)discard;}vec3 p=atlasSphere(weights,a,b,c,bias,blend);
 ...
 float areaLog=clamp(log((2.*PI/(3.*sqrt(3.)))/areaElement)/log(2.)/2.,-1.,1.);
 ...
 float lineAlpha=0.;vec3 lineColor=palette==2?vec3(.65,.85,.91):vec3(.31,.53,.59);
 if(grid>0.){float lo=abs(mod(lon+grid*.5,grid)-grid*.5)*max(.12,cos(lat));float la=abs(mod(lat+grid*.5,grid)-grid*.5);lineAlpha=(1.-smoothstep(gridWidth*.4,gridWidth,min(lo,la)))*.35;color=mix(color,lineColor,lineAlpha);}
 if(overlayOnly==1){
...
function rebuild(fit=true){
 ...
 if(nextKey!==geometryKey){
  ...
  const unique=[...new Set(ids)].sort();edgeLabels=tiles.map(t=>hex.map((_,e)=>String.fromCharCode(97+unique.indexOf(ids[t.id*6+e]))));
 }
...
function drawSubgrid(){
 ...
 const color=$('palette').value==='night'?'#b2e2ee':'#315865',levels=subgridLevels;
 for(const t of visible){
  ...
  if($('subgrid').checked){
   ...
   ctx.beginPath();ctx.strokeStyle=color;ctx.globalAlpha=index===0?.14:.72;ctx.lineWidth=index===0?Math.max(.5,scale*state.zoom*.002):Math.max(.8,scale*state.zoom*.0038);
...
function prepareIndicatrix(job){
 ...
 if(!indicatrixWorker){
  indicatrixWorker=new Worker(new URL('./indicatrix-worker.mjs',import.meta.url),{type:'module'});
...
function render(refined=false,exportMode=false){queued=false;syncOptionCards();updateDistortionLegend();refined=refined===true;clearTimeout(reliefRefineTimer);$('zoom-value').textContent=Math.round(state.zoom*100)+'%';if(!ready||!gl||!program)return;updateVisibleMesh();gl.bindFramebuffer(gl.FRAMEBUFFER,null);gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);gl.useProgram(program);gl.uniform1f(uniforms.gridRotation,state.gridRotation*Math.PI/180);gl.uniform2f(uniforms.size,w,h);gl.uniform3f(uniforms.view,scale*state.zoom,state.panX,state.panY);gl.uniform3f(uniforms.angles,state.lon*Math.PI/180,state.lat*Math.PI/180,state.roll*Math.PI/180);gl.uniform1f(uniforms.bias,state.bias);gl.uniform1f(uniforms.blend,+$('interpolation').value);gl.uniform1f(uniforms.grid,$('graticule').checked?state.grid*Math.PI/180:0);gl.uniform1f(uniforms.gridWidth,.6/(scale*state.zoom));gl.uniform1i(uniforms.palette,displayedSource==='continents'?['atlas','original','night'].indexOf($('palette').value):1);gl.uniform1i(uniforms.distortion,derivativeSupport?($('distortion').checked?3:0):0);gl.uniform1f(uniforms.distortionOpacity,state.distortionOpacity);gl.uniform1f(uniforms.pixelScale,scale*state.zoom*dpr);gl.uniform1i(uniforms.map,0);gl.uniform1i(uniforms.felvClip,arrangement.clip?1:0);
 ...
 if(relief?.ready&&$('relief-enabled').checked){
  ...
  try{relief.render({width:w,height:h,dpr,unit:scale*state.zoom,state,blend:+$('interpolation').value,clip:arrangement.clip,material:materialMode(),treatment:$('relief-treatment').value,tone:$('relief-tone').value,signature,drawColor:(width,height)=>drawColor(width,height,true),drawGeometry,seams,riverTexture,riverVisible:$('rivers-visible').checked,riverDepth:state.reliefRiverDepth,pixelBudget:exportMode?EXPORT_RELIEF_PIXELS:INTERACTIVE_RELIEF_PIXELS,refined});
 ...
 for(const t of visible){ctx.globalAlpha=t.opacity;ctx.beginPath();(t.polygon||hex).forEach((p,i)=>{const xy=point(p,t);i?ctx.lineTo(...xy):ctx.moveTo(...xy);});ctx.closePath();ctx.strokeStyle='#376472';if(state.line>0){ctx.lineWidth=state.line;ctx.stroke();}
...
async function exportPNG(){if(!ready||!gl||!program)return;const button=$('export'),label=button.textContent,{requested,actual}=exportScale(),saved={dpr,canvasWidth:canvas.width,canvasHeight:canvas.height,overlayWidth:overlay.width,overlayHeight:overlay.height};let restored=false;const restore=()=>{if(restored)return;restored=true;dpr=saved.dpr;canvas.width=saved.canvasWidth;canvas.height=saved.canvasHeight;overlay.width=saved.overlayWidth;overlay.height=saved.overlayHeight;draw();};button.disabled=true;button.textContent=`Rendering PNG · ${actual.toFixed(1)}×…`;try{dpr=actual;canvas.width=Math.max(1,Math.round(w*dpr));canvas.height=Math.max(1,Math.round(h*dpr));overlay.width=canvas.width;overlay.height=canvas.height;render(true,true);const out=document.createElement('canvas');out.width=canvas.width;out.height=canvas.height;const c=out.getContext('2d');c.fillStyle='#e9f0f2';c.fillRect(0,0,out.width,out.height);c.drawImage(canvas,0,0);c.drawImage(overlay,0,0);restore();const blob=await new Promise((resolve,reject)=>out.toBlob(value=>value?resolve(value):reject(Error('PNG encoding failed')),'image/png'));const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`hex-atlas-${state.method}-${Math.round(actual*100)/100}x.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);if(actual<requested)$('status').textContent=`Export capped at ${Math.round(actual*100)/100}× to fit this device's ${Math.round(out.width*out.height/1000000*10)/10} MP limit.`;}catch(error){restore();$('status').textContent='PNG export failed; the live map is still available.';console.warn('PNG export:',error);}finally{button.disabled=false;button.textContent=label;}}
...
function updateOptimizerUI(){const preset=searchPresets[state.method]?.[state.arrangement],available=Boolean(preset?.results?.length);$('optimize').disabled=!ready||!available;if(!available)$('optimize').checked=false;$('optimizer-note').textContent=available?'Only the exposed outer boundary is scored.':'Infinite tiling has no outer boundary to optimize.';}
...
function captureSettings(){
 const controls={};document.querySelectorAll('aside input[type=checkbox],aside select,aside input[type=range]').forEach(el=>{controls[el.id]=el.type==='checkbox'?el.checked:['land-classes','ocean-classes'].includes(el.id)?classCount(el.id):el.value;});
