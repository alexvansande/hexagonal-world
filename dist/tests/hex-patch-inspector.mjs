...
const triples=[offset(3,2),add(offset(3,2),[1,0]),add(offset(3,2),[0,1])],small=offset(6,3),lake=offset(15,3),island=offset(12,10),pale=new Set([...triples,small,lake,...directions.map(d=>add(lake,d))].map(key));
function classification(axial){if(key(axial)===key(island))return 0;if(pale.has(key(axial)))return 1;const [q,r]=axial,col=q+Math.floor(r/2);return +(r>=7&&col>=6||r>=9&&r<=11&&col>=3&&col<=6||r===8&&col===4);}
...
const cells=[];for(let row=0;row<=12;row++)for(let col=0;col<20;col++)cells.push(offset(col,row));
...
function patchPolygon(run){const {start,length}=run;return length<=3?[mid(start),...Array.from({length:length-1},(_,i)=>corners[(start+i+1)%6]),mid(start+length-1)]:[[0,0],...Array.from({length:length+1},(_,i)=>corners[(start+i)%6])];}
...
const features=[['Three-hex lake',triples[0]],['Detached patch',add(triples[0],[-1,1])],['Single-hex lake',small],['Seven-hex lake',lake],['Vertical run',offset(6,8)],['Horizontal run',offset(11,7)],['Island',island]];
...
function render(){
 ...
 for(const [text,at] of [['3-hex lake',[5.2,1.2]],['Single hex',[10.2,3.1]],['7-hex lake',[24.6,.7]],['Horizontal run',[18,9.4]],['Vertical run',[5.4,12]],['Island',[21.7,17.8]]]){const label=element('text',{x:at[0],y:at[1],class:'scene-label'});label.textContent=text;scene.append(label);}
 ...
 document.querySelector('#description').textContent=`Hex ${key(selected)} is originally ${own?'pale':'green'}. `+(n?`${n} consecutive ${runs[0].color?'pale':'green'} neighbors select ${n===6?'the full fill and circle':n===2?'the midpoint triangle':n===3?'the midpoint trapezoid':n===4?'the two-thirds patch':'the five-sixths patch'}.`:'No matching run of two or more: it remains an original hex.');
