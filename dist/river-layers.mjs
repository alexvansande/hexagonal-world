...
export function paintRiverMask(distances,widthScale,rgba=new Uint8ClampedArray(distances.length*4)){
 const threshold=Math.max(.5,Math.min(3,Number(widthScale)||1))*64;
 for(let i=0;i<distances.length;i++){
  const p=i*4;rgba[p]=rgba[p+1]=rgba[p+2]=255;
  rgba[p+3]=distances[i]!==255&&distances[i]<=threshold?255:0;
 }
 return rgba;
}
async function loadField(url,{signal}){
 ...
 try{
  ...
  const distances=new Uint8Array(canvas.width*canvas.height);
  for(let i=0;i<distances.length;i++)distances[i]=rgba[i*4];
  const field={width:canvas.width,height:canvas.height,distances};canvas.width=canvas.height=1;return field;
...
export class RiverFields{
 ...
 async get(value){
  ...
  request.promise=(async()=>{
   try{
    const field=await this.load(`maps/hydrorivers/v1/${this.mobile?'mobile':'desktop'}/level-${level}.png`,{signal:controller.signal});
