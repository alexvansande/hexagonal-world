...
export function loadPrintLettering(){
 if(!fontPromise)fontPromise=fetch(new URL('./pdf-lettering.json?v=earth-title-1',import.meta.url)).then(response=>{if(!response.ok)throw Error('Print lettering could not load');return response.json();}).catch(error=>{fontPromise=null;throw error;});
 return fontPromise;
}
...
export function addPrintLettering({fonts,add,streamObject,pageHeight,headingOffset=0,ink="0.098039 0.235294 0.286275"}){
 ...
 for(const key of Object.keys(fonts)){
  ...
  const descriptor=add(new Blob([`<< /Type /FontDescriptor /FontName /${font.name} /Flags ${font.name.includes('Italic')?96:32} /FontBBox [${font.bbox.join(' ')}] /ItalicAngle ${font.name.includes('Italic')?-12:0} /Ascent ${font.bbox[3]} /Descent ${font.bbox[1]} /CapHeight 700 /StemV 80 >>`]));
  ...
  resources.push(`/${name} ${id} 0 R`);
 }
 for(const [key,size,x,y] of [['title',44,36,pageHeight-86-headingOffset],['subtitle',13,38,pageHeight-111-headingOffset],['credit',18,36,24]])commands.push(text(key,fonts[key].text,size,x,y));
