...
export function loadPrintLettering(){
 if(!fontPromise)fontPromise=fetch(new URL('./pdf-lettering.json',import.meta.url)).then(response=>{if(!response.ok)throw Error('Print lettering could not load');return response.json();}).catch(error=>{fontPromise=null;throw error;});
 return fontPromise;
}
const literal=text=>'('+text.replaceAll('\\','\\\\').replaceAll('(','\\(').replaceAll(')','\\)')+')';
export function addPrintLettering({fonts,add,streamObject,pageHeight}){
 const resources=[],commands=[];
 for(const [key,size,x,y] of [['title',44,36,pageHeight-67],['subtitle',10,38,pageHeight-94],['credit',18,36,24]]){
  ...
  resources.push(`/${name} ${id} 0 R`);commands.push(`BT /${name} ${size} Tf 0.098039 0.235294 0.286275 rg 1 0 0 1 ${x} ${y} Tm ${literal(font.text)} Tj ET`);
 }
 return {resources:resources.join(' '),commands};
}
