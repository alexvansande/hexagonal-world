...
assert.equal(tourLocations.length,5,'Five entry points only');
assert.equal(new Set(tourLocations.map(p=>p.id)).size,5);
assert.deepEqual(tourLocations.map(p=>p.id),['origin-of-mankind','silk-road','iceland-to-vinland','french-polynesia','americas-exchange']);
...
assert.equal(projected.length,5,'every anchor must land in the visible Spaceship net');
for(const {location,tile,local} of projected){
 ...
 assert.equal(location.overlay,{'origin-of-mankind':'human-migrations','silk-road':'silk-road','iceland-to-vinland':'norse-voyages','french-polynesia':'polynesia','americas-exchange':'americas-exchange'}[location.id]);
}
...
console.log('Tour markers: five unique entry points, geographic round trips and default-only gating pass.');
