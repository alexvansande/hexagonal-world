import {layoutOptions,styleOptions,layoutPolygons} from '../map-options.mjs?v=lifezones-bg-1';
import {sharePair} from '../share-routes.mjs?v=lifezones-bg-1';
...
try{
 ...
 for(const layout of layoutOptions){
  ...
  for(const style of selected){
   result.textContent=`Rendering ${++count}/${selected.length*layoutOptions.length} · ${style.name} / ${layout.name}`;
   doc.querySelector(`[data-style="${style.id}"]`).click();await until(()=>!$('map-loading').textContent,'map source');await until(()=>!$('relief-enabled').checked||$('relief-status').textContent==='Lighting layers ready','lighting');await delay(950);await until(()=>!$('indicatrix-status').textContent,'Tissot');
   ...
   ctx.font='62px Baskerville, Georgia, serif';ctx.fillText('Hexagonal Earth',46,72);ctx.font='bold 14px Gotham, Arial, sans-serif';ctx.letterSpacing='1.8px';ctx.fillText('A COLLECTION OF HEXAGON BASED MAPS.',50,101);ctx.letterSpacing='0px';ctx.font='17px Baskerville, Georgia, serif';ctx.fillText(`${style.name} · ${layout.name}`,50,129);ctx.font='15px Baskerville, Georgia, serif';ctx.textAlign='right';ctx.fillText('hexagonal.earth',1150,129);
   ...
   if(new URLSearchParams(location.search).has('save')){const response=await fetch('http://127.0.0.1:4176'+pair.image,{method:'POST',body:blob});if(!response.ok)throw Error('Could not save '+pair.image);if(style.id==='lifezones'&&layout.arrangement==='dymaxion'){const png=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));const response=await fetch('http://127.0.0.1:4175/social-preview.png',{method:'POST',body:png});if(!response.ok)throw Error('Could not save default social image');}}
  }
 }
 result.textContent=`Ready · ${count} social previews`;result.dataset.status='passed';
