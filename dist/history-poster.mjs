...
export function posterLayout({map,spots,labels=[],scale=1,measure,heading=null,reservedRight=0,labelScale=1}){
 const k=scale,T=posterType,mapWidth=map.right-map.left;
 const column=mapWidth*.3,gap=mapWidth*.05,margin=mapWidth*.04,lead=T.lineHeight;
 ...
 const rulePad=T.body*k,textWidth=column-rulePad,swatch=T.legend*k*2.2;
 ...
 const boxes=spots.map(spot=>{
  ...
  return {id:spot.id,anchor:[spot.x,spot.y],height:y,lines,side:spot.x<(map.left+map.right)/2?'left':'right'};
 });
 const top=map.top-headBand,bottom=map.bottom+margin;
 const columnTop=side=>top+headBand-(heading?margin*.5:0)+(side==='right'?reservedRight:0);
 const spacing=T.body*k*2.2;
 ...
 for(const b of boxes){
  ...
  const from=[b.rule,b.y+Math.min(b.height/2,T.title*k*lead*1.5)];
  b.leader=leaderPath(from,b.anchor);
 }
 return {left,top,right,bottom:posterBottom,width:right-left,height:posterBottom-top,map,scale:k,labels:placeLabels(labels,{scale:k,labelScale,measure}),heading:heading?{x:left+margin,y:top+margin+T.heading*k,label:heading.label,date:heading.date}:null,boxes,labelScale};
}
...
export function drawPoster(ctx,layout,{routes=[],labels=[],clip=[],ink=posterInk}={}){
 ...
 for(const box of layout.boxes){
  ...
  // The rule on the box's inner side.
  ctx.strokeStyle=ink;ctx.lineWidth=1.1*k;ctx.lineCap='butt';ctx.beginPath();ctx.moveTo(box.rule,box.y);ctx.lineTo(box.rule,box.y+box.height);ctx.stroke();ctx.lineCap='round';
