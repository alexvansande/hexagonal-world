...
export function addLifezonesLegend({landCount,oceanCount,lettering,pageWidth,pageHeight,ink}){
 ...
 for(const h of layout.hexes){const vertices=Array.from({length:6},(_,k)=>{const a=(k*60-90)*Math.PI/180;return `${left+(h.x+Math.cos(a)*h.r)*scale} ${top-(h.y+Math.sin(a)*h.r)*scale}`;});commands.push(`${pdfColor(h.color)} rg ${vertices[0]} m ${vertices.slice(1).map(p=>p+' l').join(' ')} h f`);}
