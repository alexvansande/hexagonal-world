import {makeGosperFractal} from './gosper-fractal.mjs';
...
export const arrangementNames={infinite:'Infinite',flower:'Flower',dymaxion:'Fuller',bighex:'Big hex',felv:'Felv',single:'Rus One',double:'Rus Two',gosper:'Gosper Fractal'};
...
export function makeArrangement(tiles,name,nets,options={}){
 if(tiles.length<4)return {net:markEdges(tiles,nets[0])};
 ...
 const net=[[0,0],...directions].map(([dq,ds])=>({ ...tiling.at(q+dq,s+ds),x:(q+dq)*1.5,y:Math.sqrt(3)*(s+ds+(q+dq)/2)}));return name==='gosper'?makeGosperFractal(tiles,net,tiling,options):{net:markEdges(tiles,net)};
 }
