...
fs.writeFileSync(root+'dist/tests/wave-preview.html',source.replace('<head>','<head><script type="importmap">'+JSON.stringify(importMap)+'</script>'+setup).replace('<body>','<body>'+banner).replace('src="'+appSrc+'"','src="tests/wave-app.mjs'+(appSrc.includes('?')?'?'+appSrc.split('?')[1]:'')+'"'));
let app=fs.readFileSync(root+'dist/app.mjs','utf8').replaceAll("from './","from '../").replaceAll("new URL('./","new URL('../");
app="import {renderWaveLegend} from './wave-legend.mjs';\n"+app;
