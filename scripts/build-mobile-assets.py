"""Build bounded-size map textures and a static neutral relief layer for phones."""
...
z=np.asarray(h,dtype=float);dy,dx=np.gradient(z);shade=np.clip(235+dx*2-dy*2,110,255).astype('uint8');Image.fromarray(shade).save(out/'shade.jpg',quality=85)
...
print('Mobile map textures: at most 1920 × 960; static terrain shading ready.')
