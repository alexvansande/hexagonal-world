...
const fs=`${derivativeSupport?'#extension GL_OES_standard_derivatives : enable\n#define HAS_DERIVATIVES 1\n':'#define HAS_DERIVATIVES 0\n'}precision highp float;varying vec2 flatPosition;uniform int felvClip;varying float tileAlpha;varying vec3 weights;varying vec3 a;varying vec3 b;varying vec3 c;uniform sampler2D map;uniform sampler2D heightMap;uniform sampler2D bakedMap;uniform int bakedEnabled;uniform float bakedAmount;uniform vec3 bakedShadowTint;uniform vec3 bakedLightTint;uniform sampler2D riverMap;uniform vec3 angles;uniform float bias;uniform float blend;uniform float grid;uniform float gridWidth;uniform vec3 gridColor;uniform int palette;uniform int material;uniform float materialSea;uniform int riversVisible;uniform int distortion;uniform float distortionOpacity;uniform float pixelScale;const float PI=3.141592653589793;
${projectionGLSL}
${ecologyGridGLSL}
...
void main(){if(felvClip==1){float fy=-flatPosition.y;float fx=flatPosition.x-sqrt(3.)*fy;if(fy<0.||fy>sqrt(3.)||fx< -1.||fx>5.)discard;}vec3 p=mapSphere(localPosition,regionIndex,weights,a,b,c,bias,blend);
 ...
 if(bakedEnabled==1){color*=mix(vec3(1.),bakedShadowTint,clamp(baked.r*bakedAmount,0.,1.));color=1.-(1.-color)*(1.-bakedLightTint*clamp(baked.g*bakedAmount,0.,1.));}
